...
const List = ({posts}: any) => {
  ...
  const Posts = () => {
    // let newPk: string;
    return posts.map(({name, htmlStr}: any) => {
      ...
      return (
        <div
          key={newPk}
          className="w-1/5 float-left m-4 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
          <Gallery
            url={image}
            alt={`product-${newPk}`}
          />
          <div className="p-5">
              ...
              <div dangerouslySetInnerHTML={{__html: imsiCont}} className="mb-3 font-normal text-gray-700 dark:text-gray-400"></div>
